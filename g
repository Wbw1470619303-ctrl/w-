...
local Global = {
    ...
    wallhopConnection = nil, -- 新增：跳墙功能连接存储
    rainbowAnimationConnection = nil, -- 新增：彩虹动画连接存储
    flyConnection = nil -- 新增：飞行功能连接存储（Delta 保活用）
}
...
local CoreGui = game:GetService("CoreGui") -- 预加载 CoreGui 用于悬浮窗绑定
...
local function getRainbowColor(hueOffset)
    local hue = (tick() * 0.2 + hueOffset) % 1 -- 0.2 控制变色速度，值越大越快
    return Color3.fromHSV(hue, 0.8, 0.9) -- 饱和度0.8、明度0.9，颜色更鲜艳
end
...
-- 主UI创建函数（修复语法闭合+Delta适配+彩虹小窗）
function createMainUI()
    -- Delta 适配：窗口参数简化+新增最小化回调
    local MainWindow = WindUI:CreateWindow({
        ...
        Theme = "Dark",
        SideBarWidth = 220,
        OnClose = function() MainWindow:Hide() end, -- 关闭时隐藏（保留悬浮窗）
        OnMinimize = function() -- 新增：最小化回调，确保悬浮窗可见
            MainWindow:Hide()
    ...
    -- 窗口按钮样式（Delta 适配：绑定 CoreGui+彩虹色初始配置）
    local function createOpenButton()
        local openButton = MainWindow:EditOpenButton({
            ...
            StrokeThickness = 2,
            Color = ColorSequence.new(getRainbowColor(0), getRainbowColor(0.1)), -- 初始彩虹色
            Draggable = true,
            Parent = CoreGui, -- 绑定到 CoreGui，避免 Delta 回收
            ZIndex = 9999, -- 悬浮在最上层，不被遮挡
            Size = UDim2.fromOffset(120, 40), -- 调整小窗大小，便于点击
        })
        -- 启动彩虹动画
    ...
    GeneralTab:Button({
        ...
        Callback = function()
            ...
            -- Delta 适配：拆分 HttpGet 和 loadstring，双重错误捕获
            ...
            local execSuccess, wallhopFunc = pcall(function()
                ...
                return func() -- 执行脚本并获取返回值（若脚本返回连接对象）
            end)

            if execSuccess then
                -- 若脚本返回 RunService 连接，直接存储；否则创建空连接标记
                if typeof(wallhopFunc) == "RBXScriptConnection" then
                    Global.wallhopConnection = wallhopFunc
                else
                    -- 兼容脚本无返回连接的情况，用空连接标记状态
    ...
    GeneralTab:Button({
        ...
        Callback = function()
            ...
            if Global.flyConnection then
                ...
                WindUI:Notify({ Title = "关闭", Content = "超人飞行已关闭", Duration = 3 })
                return
            end

            -- Delta 适配：拆分 HttpGet 和 loadstring，双重 pcall 捕获错误
            ...
            if not success then
                WindUI:Notify({ Title = "加载失败", Content = "飞行脚本链接无法访问，请检查网络", Duration = 3 })
                return
            end

            -- 执行飞行脚本并绑定连接（适配 Delta 保活逻辑）
            local execSuccess, flyFunc = pcall(function()
                local func = loadstring(flyScript)
                return func() -- 捕获脚本返回的连接对象（若有）
            end)

            if execSuccess then
                -- 存储连接对象，便于后续关闭和 Delta 保活
